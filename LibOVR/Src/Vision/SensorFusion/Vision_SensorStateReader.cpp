...
// Following two functions are an extended version of the previous two.
// EXTENDED TO CALCULATE "PREDICTION" IN THE PAST (negative predictionDt are not cropped to zero!)
// Useful to have an estimation of the pose in a past time (not too far away..)
static Pose<double> calcPredictedPoseExtended(const PoseState<double>& poseState, double predictionDt)
{
	...
	if (fabs(predictionDt) > candidateDt)	// *** TWEAKED! ***
	{
		dynamicDt = candidateDt*Sign(predictionDt);	// must preserve the sign
	}
...
PoseState<float> calcPredictedPoseStateExtended(const LocklessSensorState& sensorState, double absoluteTime, const Posed& centeredFromWorld)
{
	...
	if (fabs(pdt) > maxPdt)	// if prediction is too in the future (or in the past), it is clamped to maxPdt
	{
		pdt = maxPdt*Sign(pdt);		// *** TWEAKED! ***
		static double lastLatWarnTime = 0;
		if (lastLatWarnTime != sensorState.WorldFromImu.TimeInSeconds)
		{
			lastLatWarnTime = sensorState.WorldFromImu.TimeInSeconds;
...
// A copy of the previous two functions, but using the extended version of pose estimation.
bool TrackingStateReader::GetTrackingStateAtTimeExtended(double absoluteTime, TrackingState& ss) const
{
	LocklessCameraState cameraState;
	LocklessSensorState sensorState;
...
bool TrackingStateReader::GetPoseAtTimeExtended(double absoluteTime, Posef& transform) const
{
	TrackingState ss;
